...
int main() {
  ...
  TgBot::InlineKeyboardMarkup::Ptr keyboardYesNo1(new TgBot::InlineKeyboardMarkup);
  ...
  if (query->data == button_datas::GroupYes1) {
      bot.getApi().sendMessage(query->message->chat->id, messages::CreatedGroup);
      // вернуться в меню
      ...
      bot.getApi().sendMessage(query->message->chat->id, messages::DeletedGroup);
      // вернуться в меню
