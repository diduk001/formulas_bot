...
int main(){
    ...
    TgBot::Bot bot("7041771878:AAHV0wS6Xkc_bn9RFbL9hn2RuQB1yzcaUiQ");

    //КЛАВИАТУРА для ответа да/нет 1
    TgBot::InlineKeyboardMarkup::Ptr keyboardGroupYesNo1(new TgBot::InlineKeyboardMarkup); // создали инлайновую клавиатуру
    ...
    keyboardGroupYesNo1->inlineKeyboard.push_back(buttons1); // заталкиваем вектор в клавиатуру
    
    //КЛАВИАТУРА для ответа да/нет 2
    TgBot::InlineKeyboardMarkup::Ptr keyboardGroupYesNo2(new TgBot::InlineKeyboardMarkup); 
    ...
    keyboardGroupYesNo2->inlineKeyboard.push_back(buttons2); 
    ...
    bot.getEvents().onCallbackQuery([&bot, &keyboardGroupYesNo1,&keyboardGroupYesNo2](TgBot::CallbackQuery::Ptr query){
    if (query->data == "Yes1") { 
    ...
    bot.getEvents().onCommand("exit_group",[&bot,&keyboardGroupYesNo2](TgBot::Message::Ptr message){
        int64_t userId=message->chat->id;
        int64_t ownerId;
        ...
        if (ownerId!=userId){
            bot.getApi().sendMessage(userId,"Вы точно хотите выйти из группы?",false,0,keyboardGroupYesNo2);
            //вернуться в меню
        }
        else{
            bot.getApi().sendMessage(userId,"Вы точно хотите удалить группу, в которой являетесь владельцем?",false,0,keyboardGroupYesNo2);
        } 
    });
    ...
    bot.getEvents().onAnyMessage([&bot,&keyboardGroupYesNo1](TgBot::Message::Ptr message){
        int64_t userId=message->chat->id;
        ...
        if (state==Group_State::WAITING_FOR_GROUP_NAME)
        {
            ...
            bot.getApi().sendMessage(userId,"Хотите ли вы добавить бота в чат?",false,0, keyboardGroupYesNo1);
